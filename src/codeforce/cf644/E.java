...
public class E {
    ...
    static boolean dfs(int i, int j, int n, Boolean[][] valid, char[][] cs){
        if (i == n || j == n) return true;
        if (cs[i][j] == '0') return false;
        if (valid[i][j] != null) return valid[i][j];
        valid[i][j] = dfs(i + 1, j, n, valid, cs) || dfs(i, j + 1, n, valid, cs);
        return valid[i][j];
    }
