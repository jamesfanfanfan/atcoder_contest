...
public class C {
    ...
    static void solve(Scanner in, PrintWriter out){
        ...
        int max = 0;
        for (int i = 0; i < n; i++) {
            if (!mp.containsKey(urr[i])) mp.put(urr[i], new ArrayList<>());
            mp.get(urr[i]).add(srr[i]);
            max = Math.max(max, mp.get(urr[i]).size());
        }
        long[] add = new long[n];
        ...
        for(int k : mp.keySet()){
            ...
            long[] pre = new long[ls.size() + 1];
            for (int i = 1; i <= ls.size(); i++) {
                pre[i] = pre[i - 1] + ls.get(i - 1);
            }
            int len = ls.size();
            for (int i = 1; i <= ls.size(); i++) {
                int mod = len % i;
