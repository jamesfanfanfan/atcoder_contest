...
public class D {
    ...
    static void solve(Scanner in, PrintWriter out){
        ...
        for (int i = 1; i < n; i++) {
            int div = i + 1;
            if (arr[i] % div == 0){
//                out.println(div+" "+1+" "+(arr[i] / div));
                res.add(new long[]{div, 1, (arr[i] / div)});
            }else{
                long dis = 0;
                if (arr[i] < div){
                    dis = div - arr[i];
...
//                out.println(1+" "+div+" "+(dis));
                res.add(new long[]{1, div, dis});
//                out.println(div+" "+1+" "+(arr[i] + dis) / div);
                res.add(new long[]{div, 1, (arr[i] + dis) / div});
            }
        }
        for (int i = 1; i < n; i++) {
//            out.println(1+" "+(i + 1)+" "+tar);
            res.add(new long[]{1, i + 1, tar});
        }
        out.println(res.size());
        for(long[] ok : res){
            out.println(ok[0]+" "+ok[1]+" "+ok[2]);
        }
    }
