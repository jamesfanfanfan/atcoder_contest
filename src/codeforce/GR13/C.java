...
public class C {
    ...
    static void solve(Scanner in, PrintWriter out){
        ...
        for (int i = 0; i < n; i++) {
            long get = dc[i];
//            arr[i] = Math.max(1, arr[i] - get);
            long val = arr[i];
            res += Math.max(0, arr[i] - get - 1);
            get -= Math.max(0, arr[i] - (n - i) + 1);
            arr[i] = Math.min(arr[i], n - i - 1);
            while (arr[i] > 1){
                int idx = (int)arr[i];
                if (idx + i >= n){
                    arr[i]--;
            ...
            dc[i + 1] += Math.max(0, get);
//            System.out.println("i:"+i+" res:"+res+" get:"+get+" val:"+val);
