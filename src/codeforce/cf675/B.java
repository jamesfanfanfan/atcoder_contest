    ...
    static void solve(Scanner in, PrintWriter out){
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                int[] dn = new int[]{n - i - 1, j};
                int[] ri = new int[]{i, m - j - 1};
                int[] cor = new int[]{n - i - 1, m - j - 1};
//                System.out.println(Arrays.toString(cor));
                if(i == n / 2 && n % 2 == 1 && j == m / 2 && m % 2 == 1){
                    continue;
                }else if (i == n / 2 && n % 2 == 1){
                    int v = arr[ri[0]][ri[1]];
                    int mid = (arr[i][j] + v) / 2;
                    tot += (Math.abs(arr[i][j] - mid) + Math.abs(v - mid));
                    seen[ri[0]][ri[1]] = true;
                }else if (j == m / 2 && m % 2 == 1){
                    int v = arr[dn[0]][dn[1]];
                    int mid = (arr[i][j] + v) / 2;
                    ...
                    int[] neo = new int[]{arr[i][j], arr[ri[0]][ri[1]], arr[dn[0]][dn[1]], arr[cor[0]][cor[1]]};
                    Arrays.sort(neo);
                    int mid = (neo[1] + neo[2]) / 2;
                    for(int v : neo){
                        tot += Math.abs(v - mid);
                    }
                    seen[dn[0]][dn[1]] = true;
                    seen[ri[0]][ri[1]] = true;
                    seen[cor[0]][cor[1]] = true;
                }
            }
        }
        out.println(tot);
