...
public class Main {
    ...
    static void solve(Scanner in, PrintWriter out){
        ...
        for (int i = 0; i < n; i++) {
            mp.put(crr[i], mp.getOrDefault(crr[i], 0) + 1);
        }
        Map<Integer, Integer> le = new HashMap<>();
        for (int i = 0; i < n; i++) {
            le.put(arr[i], le.getOrDefault(arr[i], 0) + 1);
        }
        long cnt = 0;
        for (int i = 0; i < n; i++) {
            long r = mp.getOrDefault(i, 0);
            long l = le.getOrDefault(brr[i], 0);
            cnt += r * l;
        }
        out.println(cnt);
    }
